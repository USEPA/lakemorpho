...
Recent work has demonstrated the ability to estimate many of these metrics from ubiquitous spatial data.  For instance, maximum depth and lake volume may be predicted using the lake polygon and surrounding topography [@hollister2010volume; @hollister2011predicting]. The initial development of these tools were developed with proprietary tools thus limiting their use.  In an effort to 
...
Using R as a Geographic Information System is now possible as several packages provide spatial data handling, geospatial analysis, and visualization.  It is becuase of these pacakges that I chose to implement `lakemorpho` as an R package.  In particular,`lakemorpho` relies on the following packages:

List here:

In addition to these packages two external libraries, Geospatial Data Abstraction Library (GDAL) and the Geometry Engine, Open Source(GEOS), are needed and their availability to R and installation varies by operating system [@GDAL2012;@GEOS2013].   
