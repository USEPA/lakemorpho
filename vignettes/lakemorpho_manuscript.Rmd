...
Using R as a Geographic Information System is now possible as several packages provide spatial data handling, geospatial analysis, and visualization.  It is becuase of these pacakges that I implemented `lakemorpho` as an R package.  In particular,`lakemorpho` relies on the following packages: `maptools`, `rgdal`, `raster`, `rgeos`, `sp`, `geosphere`[@bivand2014maptools;@bivand2014rgdal;@bivand2014rgeos;@bivand2008applied;@pebesma2005sp;@hijmans2014raster;@hijmans2014geosphere].  In addition to these packages two external libraries, the Geospatial Data Abstraction Library (GDAL) and Geometry Engine, Open Source(GEOS), are needed.  Their availability to R and installation varies by operating system [@GDAL2012;@GEOS2013].  

DEPENDENCIES (create from what was used to write this paper, plus a rebuild.)

##Using `lakemorpho`

Included in `lakemorpho` are, one function to create a `lakeMorpho` object,  eleven functions to caclulate morphometry metrics, a default plotting object, two example datasets, and an example `lakeMorpho` object.

A typical workflow for using `lakemorpho` to calculate lake metrics would include
pulling data into R, creating lakemorpho class (+ plot.lakeMorpho  Default plotting of a lakeMorpho object),  and calculating metrics.  The following sections provide details on the functions and illustrate the use of `lakemorpho` with the provided example data.

###The `lakeMorpho` Class and `lakeSurroundTopo`  
In order to estimate maximum depth, mean depth, and volume information about the surrouding topography and the maximum in-lake distance to shoreline is needed [@hollister2011predicting; @hollister2010volume].  The `lakeSurroundTopo` function calculates this information from two required inputs, a `raster` object of elevation and a `SpatialPolygons` object of the lake.  By default the surrouding topography is a buffer of the lake shoreline equal to the maximum in-lake distance.  An optional `SpatialPolygons` object of the catchements (or any polygon intersecting the lake) can be used to define the surrounding topography instead of the default buffer.  Both raster and vector represenations are used in `lakemorpho`.  The default raster size is taken from the resolution of the input elevation data.

All of the functions of `lakemorpho` assume that projections have been handled prior to creating the `lakemorpho` class or calculating the metrics.  If the input data are not of the same projection, `lakeSurroundTopo` will return an error.  The data must be reprojected into the same spatial reference.
...
To access the help pages (including a version of this manuscript) use `r help(package='lakemorpho')`.  
