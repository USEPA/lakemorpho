...
lakeSurroundTopo<-function(inLake,inElev,inCatch = NULL,reso = res(exampleElev)[1])
{
  ...
  if(gArea(inLake)<=8100){return(NULL)}
  ...
  lakepr<-rasterize(SpatialPolygons(inLake@polygons),
                    raster(xmn=xmin,xmx=xmax,
                           ymn=ymin,ymx=ymax,
  ...
  if(inLakeMaxDist<100){inLakeMaxDist<-100} 
  ...
  {
    outputR<-raster(extent(xSurround),
                    ncols=round((extent(xSurround)@xmax-extent(xSurround)@xmin)/res(inElev)[1]),
                    nrows=round((extent(xSurround)@ymax-extent(xSurround)@ymin)/res(inElev)[1]),
                    crs=CRS(proj4string(xSurround)))
  ...
  #  lakeOnEdge<-T
  #}  
  return(lakeMorpho(inLake,xElev,xSurround,xLakeDist,lakeOnEdge))
}